...
def main():
    ...
    ap.add_argument("--centres", action="store_true",
                    help="use way centroids, the pre-geometry behaviour")
    a = ap.parse_args()

    cores = osm_cores()
    assign = themes.load()
    ni = neighbours.NeighbourIndex(radius_m=a.radius, use_geometry=not a.centres)
    ...
    n = len(cores)
    print(f"radius {a.radius:.0f} m | {n} cores | source: {ni.source}\n")
    ...
    # Bigger neighbour lists are not automatically better: the model invented a
    # British theme on round-2 item 68 out of streets in an ADJACENT subdivision.
    # Growth here is the cost side of the geometry switch.
    counts = sorted(len(ni.near(k)) for k in cores)
    def q(p):
        return counts[min(len(counts) - 1, int(len(counts) * p))]
    print(f"\n  neighbours per street: median {q(0.5)}, p90 {q(0.9)}, "
          f"p99 {q(0.99)}, max {counts[-1]}")
    over = sum(1 for c in counts if c > 22)
    print(f"  over the prompt limit of 22: {over} ({over/n:.1%})")
